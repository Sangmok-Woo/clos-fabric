...
> **한 줄** — 대시보드 구축 자체가 목적이 아니라, **장애를 모니터링이 알아채는 데 몇 초 걸리는지를 실측**하고,
...
- **exporter** — 제3자 바이너리(frr_exporter) 대신 **직접 쓴 경량 수집기**(`exporter/collector.py`).
  ...
  사이드카·소켓 공유가 필요 없어 배포가 단순하고, 직접 이해하고 고칠 수 있는 스크립트를 쓴다는 방침과도 맞는다.
...
2. **핵심 알람은 붙어야 할 세션이 안 붙은 상태를 잡는다**.
   `clos_bgp_peers_established < clos_bgp_peers_expected`. RULES에서 걱정했던
   붙어야 할 리프가 빠져도 사람이 못 알아챈다는 문제의 답이 이 한 줄이다.
...
**spine2 경유 세션만 빨갛게** 끊기고 spine1 경로는 초록으로 유지된다 — 한 대가 죽어도 절반은 살아 있다.
...
- **WSL 재시작 = veth 소실** — 베이스 랩이 `Active`로 떠 있으면 `up.sh`가 재배포하니 문제없다. 랩 쓰는 동안 WSL 세션을 하나 열어둘 것.

...
랙마다 서브넷이 다른 것이 Clos의 기본형이지만, 실무에서는 랙이 달라도 같은 서브넷이어야 하는 요구(VM 이동, 클러스터 브로드캐스트)가 있다.
...
확장 규칙을 정하며 갈림길 두 개를 랩에서 직접 돌려보고 결정했다. 판단 기준은 **혼합 상태에서 문제가 없는가, 설정이 실제로 단순해지는가** 두 가지였다.
...
대역 안이라면 아무 장비나 붙을 수 있다는 걱정도 unnumbered에서는 자연 해소된다 — link-local은 라우팅되지 않으므로 그 포트에 꽂힌 장비만 붙는다.

> 검증까지 하고 버린 기록을 남기는 이유: **기각도 결정이다.** 시간 순 결정 기록은 [CHANGELOG.md](../CHANGELOG.md).

## 5. 관측 — 장애를 알아채는 데 걸리는 시간
...
이 시나리오의 관측 설계에는 **기대값을 상수로 박지 않는다는 DESIGN 원칙을 그대로 적용**했다:
